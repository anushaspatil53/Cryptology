...
def shift_cipher_encrypt(text, shift):
    ...
    for char in text:
        if char.isalpha():
            # Determine the start of the alphabet (uppercase or lowercase)
            start = ord('A') if char.isupper() else ord('a')
            # Encrypt the character and handle wrapping around the alphabet
...
def brute_force_decrypt(cipher_text):
    # Try all possible shift values from 1 to 25 (0 is a trivial case)
...
def atbash_cipher(text):
    # Create a mapping dictionary for the Atbash cipher
    alphabet = 'abcdefghijklmnopqrstuvwxyz'
    reversed_alphabet = alphabet[::-1]  # Reversed alphabet for Atbash
    cipher_map = dict(zip(alphabet, reversed_alphabet))
    cipher_map.update(dict(zip(alphabet.upper(), reversed_alphabet.upper())))

    # Encode or decode the text using the mapping
...
# Validation
try:
    # Example of invalid key 'a' that is not coprime with 26
    invalid_a = 13
